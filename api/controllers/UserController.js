const { generateRandomString, removeEnter, denormalizeComaString, generateRandomNumber, revertWordBreak } = require('../helper/stringmod');
const { newUserMailContent, newUserMailSubject,
	newUserMailContentV2, newUserMailContentV3,
	forgotPasswordSubject, forgotPasswordContent } = require('../mail/newuser');
const { content } = require('../mail/email');
const { createToken, createTokenVerify, userFirstLastName, googleTokenCheck } = require('../helper/userHelper');
...
const User = {
	get: async function (req, res) {
		...
		const users = await models.User.find(filter, attributes, sort).populate(populate)
	...
	getDetail: async function (req, res) {
		...
		return response.ok(data, res, i18n(`Success`, {}, req.headers['accept-language'], 'general'));
	},
	...
	add: async function (req, res) {
		...
		const existing_email = users.find(item => item.email.trim().toUpperCase() == email.trim().toUpperCase());
		if (existing_email) {
			return response.error(400, `Email already used by another user`, res, `Email already used by another user`);
		}

		if (phone_number) {
			const existing_phone_number = users.find(item => item?.phone_number?.trim().toUpperCase() == phone_number.trim().toUpperCase());
	...
	register: async function (req, res) {
		...
		const { email, password, confirm_password, phone_number, first_name, last_name, organization_id, dob } = req.body;
		...
		if (password != confirm_password) return response.error(400, `Password not match`, res, `Password not match`);
		if (password.length < 7 || confirm_password.length < 7) return response.error(400, `Password should min. 7 character`, res, `Password should min. 7 character`);
		...
		try {
			...
			//send email
			const body_user = await content(user.email, organization_id, 13, { name: user.name, password, link });
			if (body_user) await models.PendingMail(body_user).save(options);
	...
	login: async function (req, res) {
		...
		if (user_obj?.role_id) user_obj.role_id.permissions = user_obj?.role_id?.permissions.sort((a, b) => a.page_id.order > b.page_id.order ? 1 : -1);
		...
		const data = {
			token: await createToken(user, organization_id, process.env.SECRET, '30 days'),
			user: user_detail
		};
	...
	update: async function (req, res) {
		const current_date = moment().tz('Asia/Jakarta').format();
		const { user_id, name, email, addresses = [],
			phone_number, dob, active_status, role_id,
			description, gender, member_id } = req.body;

		let flag_update_password = 0;

		if (!user_id) return response.error(400, `User not found`, res, `User not found`);
		...
		if (flag_update_password) {
			const body_user = await content(user.email, req.me.organization_id, 14, { name: user.name, original_password });
	...
	loginThirdParty: async function (req, res) {
		...
		if (!organization_id) {
			return response.error(400, `(403) Something wrong, please contact our admin.`, res, `(403) Something wrong, please contact our admin.`);
		}

		if (accessToken) {
			const auth_user = await googleTokenCheck(accessToken);
			if (auth_user?.status == 400 || !auth_user?.data) return response.error(400, `(403) Something wrong, please check your email.`, res, `(403) Something wrong, please check your email.`);
		}
		...
		const user = users.find(item => item.email.trim().toLowerCase() == email.trim().toLowerCase());
		if (user) {
			//login
			const full_name = userFirstLastName(user);
			const user_detail = {
			...
			data = {
				token: await createToken(user, user?.organizations[0]?.organization_id, process.env.SECRET, '30 days'),
				user: user_detail
			};
			return response.ok(data, res, i18n(`Success`, {}, req.headers['accept-language'], 'general'));
		}
			...
			try {
				...
				//send email
				const body_user = await content(user.email, organization_id, 2, { name: user.name });
				if (body_user) await models.PendingMail(body_user).save(options);
	...
	sendOtpForgotPassword: async function (req, res) {
		...
		if (user.forgot_password_token == null) {
			...
			user.forgot_password_expired_time = moment().tz('Asia/Jakarta').add(1, 'minutes').toDate();
			await user.save();
		}

		//send email
		const body_user = await content(user.email, user?.organizations[0]?.organization_id, 1, { name: user.name, token: user.forgot_password_token });
		if (body_user) await models.PendingMail(body_user).save();
	...
	verifyOtpForgotPassword: async function (req, res) {
		...
		if (user.forgot_password_token == null) {
			return response.error(400, `Invalid OTP code. Please try again`, res, `Invalid OTP code. Please try again`);
		}
	...
	resetPassword: async function (req, res) {
		...
		if (!password || !confirm_password) return response.error(400, `Please input your new password`, res, `Please input your new password`);
		...
		if (!user) return response.error(400, `User not found`, res, `User not found`);
		if (user.forgot_password_token == null) return response.error(400, `Invalid OTP code. Please try again`, res, `Invalid OTP code. Please try again`);
		if (user.forgot_password_token != token) return response.error(400, `Invalid OTP code. Please try again`, res, `Invalid OTP code. Please try again`);
		if (password != confirm_password) return response.error(400, `Password not match`, res, `Password not match`);
		if (password.length < 7 || confirm_password.length < 7) return response.error(400, `Password should min. 7 character`, res, `Password should min. 7 character`);
	...
	changePassword: async function (req, res) {
		...
		if (password != confirm_password) {
			return response.error(400, `Password not match`, res, `Password not match`);
		}
